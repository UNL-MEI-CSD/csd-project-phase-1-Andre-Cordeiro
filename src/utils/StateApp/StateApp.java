...
public class StateApp {
	...
	public Status getOperationStatus(UUID rID){

		if (opers.containsKey(rID)){
			return opers.get(rID);
		} else {
			return OperationStatusReply.Status.UNKOWN;
		}
	...
	public String countOperationStatus(){
		...
		for (OperationStatusReply.Status status : opers.values()) {
			if (status == OperationStatusReply.Status.PENDING){
				...
				unkown++;
			} else if (status == OperationStatusReply.Status.FAILED){
				failed++;
			} else if (status == OperationStatusReply.Status.REJECTED){
				rejected++;
			}
		}
